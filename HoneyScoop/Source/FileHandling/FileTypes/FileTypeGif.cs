...
namespace HoneyScoop.FileHandling.FileTypes {
	internal class FileTypeGif : IFileType {
		...
		private readonly struct ColorTable {
		...
		private readonly ref struct GraphicsControlExtension {
			...
			public bool IsValidFileType(ReadOnlySpan<byte> fileData) {
				...
				if(!fileData.Slice(0, HeaderSize).SequenceEqual(HeaderBytes)) {
					return false;
				}

				// Check footer terminator
				if(!fileData.Slice(fileData.Length - FooterSize, FooterSize).SequenceEqual(FooterBytes)) {
					return false;
				}
			...
			private struct FileTypeInfo {
				...
				internal FileTypeInfo(string type, string description, string version) {
					Type = type;
