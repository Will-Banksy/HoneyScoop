...
namespace HoneyScoop.FileHandling.FileTypes
{
    internal class FileTypeJpg : IFileType
    {
        ...
        private readonly ref struct Segment
        {
        ...
        public FileType FileType => FileType.Jpg;
         //finish this  part
        public FileInformation Analyze(byte[] data)
        {
            ...
            {
                var segment = Segment.Parse(data.AsSpan(), pos);
                if (segment == null)
                {
                    break;
                }
            ...
            return new FileInformation(FileType, isValid, segments.Count, data.Length, HeaderSize, FooterSize);
        }